...
export default function SearchBar() {
    ...
    useEffect(() => {
        if (debouncedQuery.length > 2) {
            axios
                .get(`http://127.0.0.1:8000/search/?query=${debouncedQuery}`)
                .then((res) => {
                    setResults(res.data);
                    setOpen(true);
                });
    ...
    return (
        <div ref={wrapperRef} className="relative w-full max-w-md mx-auto">
            <div className="flex items-center bg-gray-100 w-full p-2 text-sm">
                <input
                    type="text"
                    placeholder="Пошук..."
            ...
            {open && results.length > 0 && (
                <ul className="absolute flex flex-col gap-2 top-full shadow-2xl left-[-85px] w-[375px] bg-white rounded mt-2 p-5 pt-0 overflow-y-auto max-h-[550px] z-10">
                    ...
                    <button className="text-sm hover:underline border p-3">
